...
export default function EditEmployeeScreen() {
  ...
  const loadDepartments = async () => {
    try {
      ...
      const response = await callApi({ method: 'GET', url: '/departments/' });
  ...
  const loadEmployee = async () => {
    try {
      ...
      setEmail(data.email || '');
      setRole(data.role || '');
      setDepartment(typeof data.department === 'string' ? data.department : data.department?._id || '');
      setPosition(data.position || '');
      // Map backend status to jobType label for UI
      ...
      setHireDate(parseDate(data.hireDate));
      setProbationDate(parseDate(data.probationDate));
      setSalary(data.salary || 0);

      setDateOfBirth(parseDate(data.dateOfBirth));
      setGender((data.gender || '').charAt(0).toUpperCase() + (data.gender || '').slice(1));
      setMaritalStatus((data.maritalStatus || '').charAt(0).toUpperCase() + (data.maritalStatus || '').slice(1));
      ...
      if (data.profileImage) {
        setExistingProfileUrl(data.profileImage);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Organization (required by backend, same logic as create)
      ...
      console.log('EditEmployeeScreen - currentUser:', currentUser);
      console.log('EditEmployeeScreen - organizationId:', organizationId);
      ...
      // Debug log (without files) to compare with web payload
      console.log('EditEmployeeScreen - updating employee with payload:', {
        firstName,
        lastName,
        email,
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.formContainer}>
          ...
          <View style={styles.profileImageContainer}>
            <View style={styles.avatarContainer}>
              {profileImage ? (
                ...
                <Image source={{ uri: existingProfileUrl }} style={styles.avatar} />
              ) : (
          ...
          <FormSection title="Skills">
            <LabelInputField label="" values={skills} onChange={setSkills} />
          </FormSection>
          ...
          <FormSection title="Employee Documents (optional)">
            <View style={styles.documentsGrid}>
              <View style={styles.documentColumn}>
                <DocumentUploadCard title="ID Card Document" value={idCard} onChange={setIdCard} />
              </View>
              <View style={styles.documentColumn}>
                <DocumentUploadCard title="Degree Document" value={degreeDoc} onChange={setDegreeDoc} />
              </View>
              <View style={styles.documentColumn}>
                <DocumentUploadCard title="Certificate Document" value={certificate} onChange={setCertificate} />
              </View>
              <View style={styles.documentColumn}>
                <DocumentUploadCard title="Experience Document" value={experienceDoc} onChange={setExperienceDoc} />
              </View>
            </View>
          </FormSection>
        </View>
