...
const EditDepartmentScreen: React.FC = () => {
  ...
  const getAdminDisplay = () => {
    ...
    if (!selectedUser) {
      // Try to find from department admin
      if (department?.admin) {
        return department.admin.name || department.admin.email || 'No admin selected';
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response?.success !== false) {
        ...
        Alert.alert('Error', response?.message || response?.error || 'Failed to update department');
      }
    } catch (error: any) {
      console.error('Error updating department:', error);
      Alert.alert('Error', error?.response?.data?.error || error?.message || 'Failed to update department');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.formCard}>
        ...
        <View style={styles.fieldContainer}>
          ...
          {(profilePicture || department?.profilePicture) && (
            <View style={styles.imagePreview}>
              <Image
                source={{ uri: profilePicture?.uri || department?.profilePicture }}
          ...
          {!profilePicture && department?.profilePicture && (
            <Text style={styles.helpText}>Upload a new image to replace the current one</Text>
          )}
        </View>
